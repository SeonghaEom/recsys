...
class GRU4REC(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=1, final_act='tanh',
                 dropout_hidden=.5, dropout_input=0, batch_size=50, embedding_dim=-1, embedding_pretrained = None, use_cuda=False):
        ...
        self.h2o = nn.Linear(hidden_size + embedding_dim, output_size) ## This is linear layer for predicted embedding --> purchase item
        self.lastlayer = nn.Linear(output_size, 19020)
        self.create_final_activation(final_act)

        weight = torch.FloatTensor(embedding_pretrained)
        self.look_up = nn.Embedding.from_pretrained(weight, freeze=False, max_norm=1, norm_type=2.0, )
    ...
    def forward(self, input, hidden, flag):
        ...
        ## concat embedding
        # indices = input.unsqueeze(0)
        embedded = self.look_up(input)

        output = torch.hstack((output, embedded)) # (B, H+embedding_dim)

        output = self.h2o(output)
        if flag:
            output = self.lastlayer(output)
        logit = self.final_activation(output)
        # print("logit size: ", logit.shape) #(B, num train items) tr purchased
