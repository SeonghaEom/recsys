...
class MSGIFSR(nn.Module):
    ...
    def __init__(self, num_items, datasets, embedding_dim, embedding_pretrained=None, num_layers=1, dropout=0.0, reducer='mean', order=3, norm=True, extra=True, fusion=True, device=th.device('cpu')):
        super().__init__()

        if embedding_pretrained != None:
            weight = th.FloatTensor(np.load(embedding_pretrained))
            self.embeddings = nn.Embedding.from_pretrained(weight, freeze=False, max_norm=1, norm_type=2.0, )
        else:
