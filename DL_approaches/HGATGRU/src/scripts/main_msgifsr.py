...
parser.add_argument(
    '--dataset_dir', default='../datasets/sample', help='the dataset directory'
)
...
parser.add_argument(
    ...
    type=str,
    default='../datasets/FA_256_feat.npy',
    help='load pretrained embeddiing.',
)
...
train_sessions, testl_sessions, testf_sessions, num_items = read_dataset(dataset_dir)
# num_items += 5
...
testl_set = AugmentedDataset(testl_sessions)
testf_set = AugmentedDataset(testf_sessions)
print(len(train_set))
print(len(test_set))
print(len(testf_set))
...
#     embedding_pretrained = np.load(args.embedding_pretrained)
model = MSGIFSR(num_items, args.dataset_dir, args.embedding_dim, args.embedding_pretrained, args.num_layers, dropout=args.feat_drop, reducer=args.reducer, order=args.order, norm=args.norm, extra=args.extra, fusion=args.fusion, device=device)
